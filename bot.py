...
# Функция для расширения промта синонимами и описаниями
def expand_prompt_synonyms(text):
    ...
    # Анал
    if contains(["анальный секс", "анал", "анус", "анальный", "анальная фистинг", "анальная стимуляция"]):
        additions.append(
            "anal penetration, detailed anus, erotic anal play, butt, rimjob, erotic buttocks, backdoor play, rear entry"
        )

    # Дилдо и синонимы
    if contains(["дилдо", "большой дилдо", "конский дилдо", "огромный дилдо", "гигантский дилдо", "фаллоимитатор"]):
        additions.append(
            "large realistic dildo, dildo penetration, rubber dildo, phallic object, detailed dildo, sex toy"
        )
...
# Функция, чтобы присылать меню выбора усиления
def send_enhancer_menu(chat_id):
...
# Отправка запроса на генерацию с распознаванием и усилением
def generate_custom_image(message, enhancer):
    base_prompt = message.text
    additions = expand_prompt_synonyms(base_prompt)
    
    # Формируем финальный промт с nsfw и расширениями
    prompt = "nsfw, explicit, " + base_prompt
    if additions:
        prompt += ", " + additions
    ...
    if error:
        ...
        send_enhancer_menu(message.chat.id)  # Меню после ошибки тоже полезно показать
        return
    ...
    for _ in range(25):
        ...
        if res.status_code != 200:
            ...
            send_enhancer_menu(message.chat.id)
            return
        status = res.json()
...
# Обычный текст без выбора усиления — генерация с базовым промтом (без усиления)
